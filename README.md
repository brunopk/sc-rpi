...
**Control WS2812B LED strips connected to a Raspberry Pi with the Home Assistant web interface or the mobile application via its [MQTT integration](https://www.home-assistant.io/integrations/mqtt/).** SC RPi also provides a simple **MQTT-based API** to send [commands](doc/commands.md) directly, for example, using a MQTT client such as [MQTT Explorer](https://mqtt-explorer.com/).
